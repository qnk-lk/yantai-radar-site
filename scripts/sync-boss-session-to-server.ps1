param(
  ...
  [string]$LocalOutput = ".tmp/boss-session.json",
  [string]$RemotePath = "/home/ubuntu/yantai-radar-site/secrets/boss-session.json"
)
...
Write-Host "Exporting BOSS session from local browser..."
& node (Join-Path $ProjectRoot "scripts\\export-boss-session.mjs") --output $LocalPath
if ($LASTEXITCODE -ne 0) {
  throw "Failed to export local BOSS session."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "Failed to upload BOSS session to server."
}
