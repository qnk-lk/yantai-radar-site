param(
  [int]$Port = 9223,
  [string]$ProfileDir = ".tmp/boss-chrome-profile",
  [string]$Url = "https://login.zhipin.com/"
)
...
try {
  ...
  try {
    ...
    Write-Host "Opened BOSS in the debugging Chrome window: $Url"
  } catch {
...
Write-Host "Login to BOSS in this Chrome window, then run: pnpm boss:collect"
