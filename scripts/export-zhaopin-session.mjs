...
class CdpClient {
  ...
  async connect() {
    ...
    const target =
      targets.find((item) => item.type === "page" && item.url.includes("zhaopin.com")) ||
      targets.find((item) => item.type === "page");

    if (!target?.webSocketDebuggerUrl) {
      throw new Error("No Chrome page target found. Login to Zhaopin first, then rerun export.");
    }
...
async function main() {
  const outputPath = normalizePath(
    ...
    path.join(".tmp", "zhaopin-session.json")
  );
  ...
  try {
    ...
    const cookies = (result.cookies || []).filter(
      (cookie) =>
        ...
        (cookie.domain.includes("zhaopin.com") || cookie.domain.includes("zhaopin.cn"))
    );
