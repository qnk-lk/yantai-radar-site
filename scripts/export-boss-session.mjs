...
class CdpClient {
  ...
  async connect() {
    ...
    const target =
      targets.find((item) => item.type === "page" && item.url.includes("zhipin.com")) ||
      targets.find((item) => item.type === "page");

    if (!target?.webSocketDebuggerUrl) {
      throw new Error("No Chrome page target found. Login to BOSS first, then rerun export.");
    }
...
async function main() {
  const outputPath = normalizePath(
    ...
    path.join(".tmp", "boss-session.json")
  );
  ...
  try {
    ...
    const cookies = (result.cookies || []).filter(
      (cookie) =>
        ...
        (cookie.domain.includes("zhipin.com") || cookie.domain.includes("kanzhun.com"))
    );
