...
INPUT_PATH="${INPUT_PATH:-/tmp/openclaw-recruitment-run.json}"
...
RECRUITMENT_CRON_NAME="${RECRUITMENT_CRON_NAME:-烟台 / 青岛招聘信号反推线索}"
ALLOWED_CITIES="${ALLOWED_CITIES:-烟台,青岛}"
MAX_COMPANIES="${MAX_COMPANIES:-10}"
OUTPUT_PATH="${OUTPUT_PATH:-/var/www/qn-message.com/recruitment-leads.json}"
TEMP_DIR="${TEMP_DIR:-/tmp/yantai-radar-openclaw}"
TEMP_JSON="$TEMP_DIR/recruitment-leads.from-openclaw.json"
STATE_DIR="${STATE_DIR:-$TEMP_DIR/state}"
STAMP_FILE="$STATE_DIR/recruitment-input.stamp"
...
if [[ -f "$INPUT_PATH" ]]; then
  if [[ ! -s "$INPUT_PATH" ]]; then
    echo "recruitment input is empty, removing stale file and falling back to cron runs"
    rm -f "$INPUT_PATH"
  else
...
if [[ -f "$OPENCLAW_JOBS_PATH" ]]; then
  CRON_JOB_ID="$(python3 - <<'PY' "$OPENCLAW_JOBS_PATH" "$RECRUITMENT_CRON_NAME"
import json
import sys
...
PY
  ...
  if [[ -z "$CRON_JOB_ID" ]]; then
    echo "recruitment cron job not found: $RECRUITMENT_CRON_NAME"
    exit 0
  fi
  ...
  if [[ ! -f "$CRON_RUNS_FILE" ]]; then
    echo "recruitment cron runs file not found: $CRON_RUNS_FILE"
  else
...
if [[ -n "$INPUT_STAMP" && ( -z "$CRON_TIMESTAMP" || "$INPUT_MTIME_MS" -gt "$CRON_TIMESTAMP" ) ]]; then
  ...
  echo "recruitment input and cron runs are both unavailable, skip"
  exit 0
fi
...
if [[ "$CURRENT_STAMP" == "$LAST_STAMP" ]]; then
  echo "recruitment input unchanged, skip"
  exit 0
fi

if [[ "$USE_INPUT_PATH" == "true" ]]; then
  python3 "$SCRIPT_DIR/openclaw_recruitment_run_to_json.py" \
    --input "$INPUT_PATH" \
    --output "$TEMP_JSON" \
...
node "$PROJECT_ROOT/server/import-document.mjs" \
  ...
  --input "$TEMP_JSON" \
  --source "openclaw-recruitment-run"
